...
def genders_index_html(): # 页面静态化
    print('正在生成首页',time.time())
    categories = {}
    ...
    index_template = loader.get_template('index.html')  # 加载模板文件

    index_template_data = index_template.render(context)  # 渲染的数据

    full_path = r'C:\Users\Administrator\Desktop\front_end_pc\index.html'

    with open(full_path, 'w', encoding='utf-8') as f:
        f.write(index_template_data)
...
def start():
    ...
    scheduler.add_job(genders_index_html, 'interval', minutes=60)  # 每五分钟运行一次 your_job 函数
    scheduler.start()
